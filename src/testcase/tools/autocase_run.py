...
def interfaceTest(case_list, file_path):
    ...
    for case in case_list:
        ...
        if method.upper() == 'GET':
            ...
            '''
            print "用例：",case_id,",postdata==",postdata
            results = requests.post(url,data=postdata).text
...
def copy_excel(file_path, res_flags, request_urls, responses):
    ...
    new_book.save('../../report/%s_测试结果.xls'.decode('utf-8') % time.strftime('%Y%m%d%H%M%S'))
...
if __name__ == '__main__':
    ''''' 
    然后进行调用，调用的时候需要传入一个excel，调用方式是 excel路径
    如果文件错误的话，会提示错误，如果正确的话， 
    会调用读excel的程序，执行用例，运行完成后，会打印Done 
    '''
    try:
        filename = u"接口_testcase.xlsx"
    except IndexError, e:
        print '请传入正确的testcase文件'
    else:
        readExcel(filename)
    print 'Done!'